...
class DataBase:
    ...
    def select_table_models(self):
        sql = '''
            SELECT brands.brand_name, models.model_name, colors.color_name FROM models
    ...
    def select_customers_by_country(self):
        sql = '''
            SELECT country, COUNT(country) FROM customers GROUP BY country ORDER BY count;
        '''
        return self.manager(sql, fetchall=True)
    ...
    def select_brands(self):
        sql = '''
            SELECT brands.brand_name, COUNT(models.model_name) FROM models 
            JOIN brands USING(brand_id)
    ...
    def select_models_price(self):
        sql = '''
            select SUM(model_price) AS all_models_price from models;
        '''
